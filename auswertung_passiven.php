...
<html>
<head>
  <title><?=$_config_title ?> - Auswertung Passiven</title>
...
<?
if(!$bilanz_date) 
	die("<div style=titel>Passiven</div><br><form method=POST action='$PHP_SELF'>Stichdatum: <input type=text name=bilanz_date style='width:100px' value=\"".date("d.m.Y")."\"> <input type=submit value='Generieren'></form></body></html>");?>	
...
<?
	...
	printLine("","<span style=\"font-size:13px;font-weight:bold\">Passiven</span>","","","","","");
	printLine("","","","","","","");

	//Nebenkonten
  $query=mysql_query("SELECT ne.id,ne.name,sum(bu.betrag*bu.kurs) FROM $buchhaltung"."_Nebenkonto ne LEFT JOIN $buchhaltung"."_Konto ko ON ne.id = ko.nebenkonto LEFT JOIN $buchhaltung"."_Buchungssaetze bu ON (bu.kt_haben = ko.nr) WHERE ne.typ=2 AND bu.datum <= '".date_CH_to_EN($bilanz_date)."' GROUP BY ne.id ORDER by ne.id");
  while(list($nr,$name,$betrag)=mysql_fetch_row($query)) {
    $query2=mysql_query("SELECT ne.id,ne.name,sum(bu.betrag*bu.kurs) FROM $buchhaltung"."_Nebenkonto ne LEFT JOIN $buchhaltung"."_Konto ko ON ne.id = ko.nebenkonto LEFT JOIN $buchhaltung"."_Buchungssaetze bu ON (bu.kt_soll = ko.nr) WHERE ne.typ=2 AND ne.id = '$nr' AND bu.datum <= '".date_CH_to_EN($bilanz_date)."' GROUP BY ne.id ORDER by ne.id");
    $betrag-=@mysql_result($query2,0,2);
    printLine("",$name,number_format($betrag,2,".","'"),"","","","");    
    $total+=$betrag;
    $total_passiven+=$betrag;
  }

	
	$query=mysql_query("SELECT kt.nr,kt.name,sum(bu.betrag*bu.kurs) FROM $buchhaltung"."_Buchungssaetze bu, $buchhaltung"."_Konto kt WHERE bu.kt_haben=kt.nr AND kt.typ=2 AND (nebenkonto is NULL OR nebenkonto <1)  AND bu.datum <= '".date_CH_to_EN($bilanz_date)."' GROUP BY kt.nr ORDER BY sort");
	for($i=1;list($nr,$name,$betrag)=mysql_fetch_row($query);$i++) {
		...
		$query2=mysql_query("SELECT sum(bu.betrag*bu.kurs) FROM $buchhaltung"."_Buchungssaetze bu WHERE bu.kt_soll ='$nr' AND bu.datum <= '".date_CH_to_EN($bilanz_date)."'");
	...
	printLine("","","","","","","");
	printLine("","<span style=\"font-size:13px;font-weight:bold\">Passiven</span>","<span style=\"font-size:13px;font-weight:bold\">".number_format($total_passiven,2,".","'")."</span>","","","","","");
?>
<tr>
