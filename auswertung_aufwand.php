...
<html>
<head>
  <title><?=$_config_title ?> - Aufwand</title>
...
<?
	...
	printLine("","","","","","","");
	$query=mysql_query("SELECT kt.nr,kt.name,sum(bu.betrag*bu.kurs) FROM $buchhaltung"."_Konto kt LEFT JOIN $buchhaltung"."_Buchungssaetze bu ON (kt.nr = bu.kt_soll AND bu.datum >= '".date_CH_to_EN($date1)."' AND bu.datum <= '".date_CH_to_EN($date2)."') WHERE kt.typ=3 AND (nebenkonto is NULL OR nebenkonto <1)  GROUP BY kt.nr ORDER BY sort");
	for($i=1;list($nr,$name,$betrag)=mysql_fetch_row($query);$i++) {
		...
		$query2=mysql_query("SELECT sum(bu.betrag*bu.kurs) FROM $buchhaltung"."_Buchungssaetze bu WHERE bu.kt_haben ='$nr'  AND bu.datum >= '".date_CH_to_EN($date1)."' AND bu.datum <= '".date_CH_to_EN($date2)."' ");
	...
	$query=mysql_query("SELECT kt.nr,sum(bu.betrag*bu.kurs) FROM $buchhaltung"."_Buchungssaetze bu, $buchhaltung"."_Konto kt WHERE bu.kt_haben=kt.nr AND kt.typ=4  AND bu.datum >= '".date_CH_to_EN($date1)."' AND bu.datum <= '".date_CH_to_EN($date2)."' GROUP BY kt.nr ORDER BY sort");
	while(list($nr,$betrag)=mysql_fetch_row($query)) {
		...
		$total_2+=($betrag-mysql_result($query2,0,0));
	}
	if($total_2-$total_1>0)
		printLine("","<b>Unternehmensgewinn</b>","<b>".number_format($total_2-$total_1,2,".","'")."</b>","","","","");
	else
		printLine("","<b>Unternehmensverlust</b>","<b>".number_format($total_2-$total_1,2,".","'")."</b>","","","","");
?>
<tr>
